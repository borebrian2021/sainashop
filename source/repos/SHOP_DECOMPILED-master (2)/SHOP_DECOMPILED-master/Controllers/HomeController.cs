...
namespace SHOP.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult payment(double ammount, int id)
        {
        ...
        public IActionResult _add_debt(  int id_finish,  double submit_price, double Total_cash_made, string date,int cust_id, double quantity_sold)
        {
            ...
            if (check_if_exists != null)
            {
                ...
                if (initial_quantity <= 0)
                {
                    ...
                    person.total = "Unable to sell the item.Stock running low";
                         return Json(person);
                ...
                else
                {
                    ...
                    double initial_cost_price_total = check_if_exists.Total_Cost_cash;
                    double total_cost_price = shop_itemss.Cost_price * quantity_sold;
                    double final_cost_price = initial_cost_price_total + total_cost_price;


                    //

                    //LETS SUBTRACT ITEMS
                    double new_quantity = initial_quantity + quantity_sold;
                    //LETS ADD SOLD ITEMS
                    double new_sold_quantity = initial_sold_quantity + quantity_sold;
                    //LETS GET INITIAL TOTAL CAS
                    double initial_total_cash = check_if_exists.Total_cash_made;
                    //LETS ADD TOTAL CASH
                    ...
                    double initial_q = change_r.Quantity;
                    double new_Q = initial_q - quantity_sold;
            ...
            //ADDING CREDITS
            var xx = _context.Creditors.FirstOrDefault(x => x.id == cust_id);
            var item = _context.Shop_items.FirstOrDefault(x => x.id == id_finish);
            var item_name = item.Item_name;
            //double init_items = item.Quantity;
            //double new_q = init_items - quantity_sold;
            //item.Quantity = new_q;
            ...
            {
                Item = item_name,
                Quantity = quantity_sold.ToString(),
                Date_created = date,
                Client_id = cust_id,
                Total = Total_cash_made,
            };
            ...
            //UPDATING ACCOUNT
            var bb = _context.Creditors.FirstOrDefault(x => x.id == cust_id);
            double initial_c = bb.Credit;
            bb.Credit = initial_c + Total_cash_made;
            ...
            person.total = "Success";

            return Json(person);
            //return RedirectToAction("creditors", "Home", new { id = cust_id });
        }

        [HttpPost]
        public IActionResult sell_Item(int id_finish, double quantity_sold, double submit_price, double Total_cash_made, string date )
        {
 ...
 CookieOptions option = new CookieOptions();  
   ...
   Response.Cookies.Append("printer_name", installedPrinterName, option);
        ...
        public IActionResult modify_stock(int mod_id, double mod_stock,string mod_name,double mod_stock_price,double mod_item_price)
        {
     ...
     public IActionResult restock(int id, int ammount,double cost_price_restock,string supplier)
        {

            var rest = _context.Shop_items.FirstOrDefault(x => x.id == id);
            double initialStock = rest.Quantity;
        ...
        public IActionResult delete_attwndant(int id)
        {
            var itemToRemove = _context.Log_in.SingleOrDefault(x => x.id == id); //returns a single item.
        ...
        public IActionResult add_debtor(String Full_name, int Phone,double credit)
        {
            //LETS JOIN TABLES
        ...
        public IActionResult add_item(String Item_name,double cost_price, double Item_price, int Quantity, String date)
        {
